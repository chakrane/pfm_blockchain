...
import PariteVerificateur from './contracts/PariteVerificateur.json';
...
function Ex5() {
  ...
  useEffect(() => {
    ...
    async function initConnection() {
      ...
      const deployedContract = PariteVerificateur.networks[netId];
      console.log(deployedContract);
      const myContract = new instanceWeb3.eth.Contract(PariteVerificateur.abi, deployedContract.address);
  ...
  async function blockChaineInfo(result, functionName){
    ...
    const estPositifFront = async() => {
        ...
        const accounts = await web3.eth.getAccounts();
        const result = await contract.methods.parite(numberInput).send({ from: accounts[0] });
        const value = await contract.methods.parite(numberInput).call();
        setNumberResult(value);
        setNumberInput('');
        await blockChaineInfo(result, 'parite(int nombre)');
        await fetchLatestBlock();
    }
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <h1 style={{ color: '#00aaff' }}>Exercice 5 : Tester la parité d'un nombre</h1>
        ...
        <div style={{
            ...
            <h3>Tester la parité d'un nombre</h3>
            <input
            type="number"
            value={numberInput}
            ...
            <button onClick={estPositifFront} style={{
                ...
                {numberResult !== '' && <p><strong>{numberResult}</strong></p>}
            </div>
        </div>
        ...
        <div style={{
          ...
          {transactionDetails && (
            <div style={{ marginTop: '2rem' }}>
              ...
              <p><strong>Nom du contrat:</strong> PariteVerificateur.sol</p>
            </div>
          )}
        </div>
      </header>
